...
window.onload = function init() {
	...
	p2 = [[vec3(-3,-3,0), vec3(-1,-3,0), vec3(1,-3,0), vec3(3,-3,0)],
		 [vec3(-3,-1,0), vec3(-1,-1,-3), vec3(1,-1,3), vec3(3,-1,-2)],
		 [vec3(-3,1,0), vec3(-1,1,-3), vec3(1,1,3), vec3(3,1,-2)],
		 [vec3(-3,3,0), vec3(-1,3,0), vec3(1,3,0), vec3(3,3,)]] ;
	p = [vec3(-3,-3,0), vec3(-1,3,0), vec3(1,3,0), vec3(3,-3,0)] ;
	gBezierCurve = new BezierCurve3(20.0,p,program) ;
	gBezierPatch1 = new BezierPatch3(12.0,p1,program) ;
	gBezierPatch2 = new BezierPatch3(20.0,p2,program) ;
...
function render() {
	...
	gPush();
	{
		...
		{
			gPush();
			var p = gBezierCurve.cpoints[i] ;
			gTranslate(p[0],p[1],p[2]);
			gScale(s,s,s) ;
			drawSphere() ;
			gPop() ;
		}
