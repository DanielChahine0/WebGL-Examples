...
function setBuffers(obj, program)
{
    ...
    // set the vertex attribute for texture coordinates
...
function BezierCurve3(n,p,prog)
{
	if( n < 1) {
		this.n = 2 ;
	} else {
...
BezierCurve3.prototype.draw = function() {
	gl.disable(gl.CULL_FACE) ;
	gl.bindVertexArray(this.vao);
...
BezierCurve3.prototype.GetVertex = function(u) {
	vd = {} ;
	...
	for( var i = 0 ; i < 4 ; i++ )
	{
		...
		// Position
		p = add(p, scalev(bi, this.cpoints[i])) ;
	}
	vd.position = vec4(p[0], p[1], p[2], 1.0) ;
	vd.colour = vec4(1.0,0.0,0.0,1.0) ;
	vd.normal = vec3(0.0,0.0,1.0) ;
	vd.texCoord = vec2(u,0.0) ;
...
BezierCurve3.prototype.SetData = function(prog){
	if( this.n < 2) return;
	
	var du = 1.0 / (this.n-1) ;
	for( var u = 0 ; u <= 1.0 ; u += du) {
		var vd = this.GetVertex(u) ;
		AddInAttribArrays(this, vd) ;
	}
