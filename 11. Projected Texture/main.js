...
function render() {
	...
	// set up projector
	{
		...
		var projScaleTrans = mult(translate([0.5,0.5,0.5]),            scale(0.5,0.5,0.5)) ; // Bias matrix
	
		// to make sure that the project is fixed in space we need to transform
		// all elements with the current
		// model matrix or post-multiply the matrix with the inverse
		var m = transpose(inverseTranspose(modelMatrix)) ;
		var ProjectorMatrix = mult(mult(projScaleTrans, mult(projProj, projView)), m);
	
		var projectorMatrixLoc = gl.getUniformLocation(program, "ProjectorMatrix") ;
		gl.uniformMatrix4fv(projectorMatrixLoc, false, flatten(ProjectorMatrix) );
